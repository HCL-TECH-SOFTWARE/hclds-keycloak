...
# Keycloak packaged by HCL Technologies

Keycloak is a high performance Java-based identity and access management solution. It lets developers add an authentication layer to their applications with minimum effort.

[Overview of Keycloak](https://www.keycloak.org/)

Trademarks: This software listing is packaged by HCL Technologies. The respective trademarks mentioned in the offering are owned by the respective companies, and use of them does not imply any affiliation or endorsement.

## Introduction

The HCL Technologies Keycloak chart heavily founds on Bitnami's charts. Bitnami charts for Helm are carefully engineered, actively maintained and are the quickest and easiest way to deploy containers on a Kubernetes cluster that are ready to handle production workloads.
...
To install the chart on local with the release name `hclds-keycloak`, run the following command in the current directory ([deployment/helm/hclds-keycloak](./)):
...
After execution, check and ensure the keycloak and related pods and services are up and running successfully
...
keycloak-0              1/1     Running   0          34m
keycloak-postgresql-0   1/1     Running   0          34m
```
...
keycloak                 ClusterIP   10.101.100.31   <none>        80/TCP,443/TCP   93m
keycloak-headless        ClusterIP   None            <none>        80/TCP,443/TCP   93m
keycloak-postgresql      ClusterIP   10.97.141.131   <none>        5432/TCP         93m
keycloak-postgresql-hl   ClusterIP   None            <none>        5432/TCP         93m
...
```console
    ...
    export HTTPS_SERVICE_PORT=$(kubectl get --namespace default -o jsonpath="{.spec.ports[?(@.name=='https')].port}" services keycloak)
    kubectl port-forward --namespace default svc/keycloak ${HTTP_SERVICE_PORT}:${HTTP_SERVICE_PORT} ${HTTPS_SERVICE_PORT}:${HTTPS_SERVICE_PORT} 
...
```console
kubectl port-forward svc/keycloak 443:443
```
output:
```
...
From here, open https://localhost or http://localhost on your browser to check it. 

## Uninstalling the Chart

To uninstall/delete the `hclds-keycloak` deployment:

```console
helm delete hclds-keycloak
```
...
```console
helm install hclds-keycloak --set auth.adminPassword=secretpassword
```
...
```console
helm install hclds-keycloak -f values.yaml
```
...
Find more information about how to deal with common errors related to the base Helm chart of Bitnami in [this troubleshooting guide](https://docs.bitnami.com/general/how-to/troubleshoot-helm-chart-issues).
